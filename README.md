...
* If you are on Windows install the `git-bash` shell when prompted during installation.
  * Git relies on some unixy features, and `git-bash` is a unixy-feeling shell provided by Git out of the box.
* This tutorial assumes a basic knowledge of navigation in a bash-like terminal.
...
* Next, let's initialize an empty git repository. This tells git that there is an active repository on your local machine and its located in your current directory:
  ...
  $: git init
  ```
  * You should see console output from Git saying: `Initializing empty Git repository in...`
* Okay great. Now we actually have a working local repository. It ain't much, but it's ours. Now its time to get to work. Let's go ahead and add our first file:
  ```
  $: touch friend-finder.js
  ```
* Wow. What a long day. We made a file. You know what? That's probably enough for now. Jerry will understand. We are at a good solid stopping point and want to save our work without having to remember what we did. Let's commit our code to our local repository.
  * The first step is check our git status. Let's double check what changed. Maybe when we went to the bathroom someone dropped a `virus.exe` into your project directory and we wouldn't want to  save that with all our hard work. So let's double check our status
    ...
    * Git status should have shown you a few different things. First your working branch, `master` a message saying `Initial commit`, and a section marked `Untracked files`.
    ##### Staging Changes:
      * Untracked files are files Git doesn't know about. That's because we haven't staged our changes yet.
    ```
    $: git add .
    ```
    * Now re-run:
    ...
    $: git status
    ```
      * You should see a section marked `Changes to be committed:` with a reference to our friend-finder.js file.
    ##### Committing changes:
    ...
    * `git commit` is the terminal command responsible for committing code. If we pass it the `-m` option, we can add a message.
      ...
      * If we only run only `git commit` a Vim text editor will be launched where you can enter a message. By saving and exiting the editor the commit command will be completed.
      * If instead you run `git commit -m` you may enter a git commit directly.
    * It's best to follow [Git Community Standards]((https://gist.github.com/digitaljhelms/3761873)) when committing code. Let's go over a few right now:
      * Commits should be __atomic__, meaning they should contain a specific focus/purpose. Much like how you would expect a food server to bill each table separately, we should commit small, frequently, and focused.
        * Think about it this way. No one wants to peer review a commit that contains 147 files with over 1,000 changes.
      * Commit messages should be __imperative__. Instead of saying "Added friend-finder.js", the commit messages should be present tense: "Add friend-finder.js"
      ...
      * Sadly, we can't always avoid big commits, and sometimes more context is needed. In these cases, provide a summary as the top level commit message, with a longer paragraph underneath detailing the change.
        * This will need to be done in vim by running the default `commit` command, so you can skip the `-m`.
    ...
    * You should see console output:
    ...
    ```
      ...
      * `0 deletions` no lines of code were removed from the file.
      * This all makes sense, friend-finder.js is an empty file.
    ##### Reviewing Changes
    * You can review your repository history using `git log`.
    * This should be pretty self-explanatory. You should see the commit hash, something like:
    ...
    ```
    * This is a unique identifier used to find your commit in the future. For more info on git log, look [here](https://git-scm.com/book/en/v2/Git-Basics-Viewing-the-Commit-History).
    ...
    * Cool. Wow is it lunch already? Let's _stage_ and _commit_ our code.
      ...
      $: git status
      ```
      * friend-finder.js has been modified and the changes are not yet staged.
      ```
      git add .
      ```
      ```
      ...
      * friend-finder.js changes are now staged.
      ...
      ```
      * Once again the console will show
      ```
      1 file changed, 3 insertions(+)
      ```
        * We added 3 lines of code, and removed no lines of code.

    #### Day 3
    * You get back the next morning and realize, you know what? That function is petty. Jerry is a good man who needs my help! So you go back in replace line 2 with:
    ```
     line 03 | console.log('Just be yourself :).');
    ```
    * Repeating the steps in day 2 _(with a new commit message)_ you should see the console output the following:
    ```
    ...
    * Git is __decentralized__.
      * Git does not require an online connection because you must always commit your code to your _local_ repository first _(We never needed the internet to make our 3 commits)_. The workflow looks like:
      ...
      * When it does come time to _push_ your code to a remote server, Git doesn't seem to care _where_ you want to store your code.
      ...
      ##### Initial Setup:
        ...
        * The most secure way of allowing your local machine to communicate with Github servers is via `SSH`. If you aren't familiar with SSH rsa key pairs here is the highly non-technical TL;DR:
          * SSH RSA key pairs allow your computer to _shake hands_ with a remote server. When the remote server recognizes sees your key, it will _trust_ your computer to push code to it via the SSH protocol. 
      ...
      * Give your remote repository a name. To keep things simple, we will give it the same name we gave the project directory, `friend-finder`.
      ...
      * Now you should be on your repo's home page. On the right side of the screen you should see a green button labeld 'Clone or download'.
        * From here Git will open a dropdown. At the top of the dropdown you will notice the option to 'use SSH' or 'use HTTPS'. If you skiped generating an SSH key, use HTTP. In my example I am going to assume you are using SSH.
      ...
      * Quickly run `git status`. Just to make sure you have no unstaged changes.
      ...
      ```
        ...
        * Now run
        ...
        $: git remote -v
        ```
        * `-v` stands for --verbose. This gives us more information than standard `git remote` which will only list remote names.
        * You should see this:
        ```
        ...
        * Say you had a team mate who came to you and said, "Hey, I just pushed some code up to Github. Want to pull that down so you have it?"
          * Assuming you have no unstaged changes, you could pull their code down from Github using: `git pull origin master`.
      ##### Wrapping Up:
      ...
      * When Rick comes around to destroy your computer, don't worry. Your code doesn't live in one place anymore. It lives in two. And because Git is _decentralizd_ it can now potentially live in many places.
      ...
      * We haven't gone over `forking`, which is where you actually __clone your own remote repository from another remote repository__. This is an invaluable tool when you need access to external Open Source packages on Github and either a) don't have time to submit a Pull Request yet or b) are waiting for a Pull Request to be approved but need to use your changes in the meantime.
        * Please note that a huge part of being a Github user is contributing and giving back. If you think changes to packages need to happen, submit a Github issue, submit a Pull Request, etc. Let everyone use your work when possible and applicable.
      * Nevertheless we did what we set out to accomplish. Our code is saved both locally and remotely, in a decentralized manner. We can now code with confidence that we can `revert` to previous commits, or re-pull our code from our remote repository if something goes terribly wrong.


Thanks for baring with me and please let me know what I can do to improve this doc. Again this is meant to help beginners who have never used Git or Github before.